...
const publicationApprovalToken = process.env.CLOUDBOX_PUBLISH_APPROVAL_TOKEN;
const remote = process.env.CLOUDBOX_E2E_GITLAB_REPO ?? "https://gitlab.cfdata.org/cloudflare/ai-agents/cloudflare-agent.git";
...
if (!owner || !internalToken || !publicationApprovalToken) {
  throw new Error("CLOUDBOX_E2E_OWNER, CLOUDBOX_INTERNAL_TOKEN, and CLOUDBOX_PUBLISH_APPROVAL_TOKEN are required");
}
const headers = { "content-type": "application/json", "x-cloudbox-internal-token": internalToken, "x-cloudbox-owner": owner };
async function call(suffix, method = "GET", body, extraHeaders = {}) {
  const response = await fetch(`${base}/api/personal-computers/${encodeURIComponent(owner)}${suffix}`, { method, headers: { ...headers, ...extraHeaders }, ...(body ? { body: JSON.stringify(body) } : {}) });
...
await call("/publication-approvals", "POST", { remote, approved: true }, { "x-cloudbox-publish-approval-token": publicationApprovalToken });
